...
static void assert_fun(FILE *fp, char *file, int line, char *expression, char *fmt, va_list ap){
...
void DGB_assert_func(char *fmt,...){
	...
	assert_fun(st_current_controller->debug_write_fp, st_current_file_name, st_current_line, st_assert_expression, fmt, ap);
	assert_fun(stderr, st_current_file_name, st_current_line, st_assert_expression, fmt, ap);
	va_end(ap);
	abort();
	
}

void static panic_func(FILE *fp, char *file, int line, char *fmt, va_list ap){
