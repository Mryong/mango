...
void set_header(Header *header, size_t size, char *filename, int line){
	header->s.size = size;
...
void set_tail(void *ptr, size_t alloc_size){
	memset((char *)ptr + alloc_size - MARK_SIZE, MARK, MARK_SIZE);
}


void check_mark_sub(unsigned char *mark, size_t size){
...
void check_mark(Header *header){
	...
	check_mark_sub((unsigned char*)header + sizeof(Header) + header->s.size, MARK_SIZE);
}
...
void *MEM_realloc_func(MEM_Controller controller, char *filename, int line,
	...
	alloc_size = sizeof(Header) + size + MARK_SIZE;
	if (ptr != NULL) {
		real_prt = (char *)ptr - sizeof(Header);
		check_mark(real_prt);
		old_header = *((Header *)real_prt);
		old_size = old_header.s.size;
		unchain_block(controller, real_prt);
	}else{
		real_prt = NULL;
		old_size = 0;
	}
	...
	new_ptr = realloc(real_prt, alloc_size);
	if (new_ptr == NULL) {
		if (ptr == NULL) {
	...
	if (ptr) {
		*((Header *)new_ptr) = old_header;
		...
		rechain_block(controller, new_ptr);
		set_tail(new_ptr, alloc_size);
	}else{
		set_header(new_ptr, size, filename, line);
		set_tail(new_ptr, alloc_size);
		chain_block(controller, new_ptr);
	}
	new_ptr = new_ptr + sizeof(Header);
...
void MEM_free_func(MEM_Controller controller, void *ptr){
	...
	check_mark(real_ptr);
	size_t size = ((Header *)real_ptr)->s.size;
	unchain_block(controller, real_ptr);
	memset(real_ptr, 0xCC, sizeof(Header) + size);
...
#ifdef DEBUG
	void *real_ptr = ptr - sizeof(Header);
	check_mark(real_ptr);
#endif
}
...
#ifdef DEBUG
	for (Header *header = controller->block_header; header; header = header->s.next) {
		check_mark(header);
	}
#endif
